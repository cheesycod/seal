...
```luau
function input.read(bytes: (number | FileSize)?, timeout: Duration?): (string?, boolean)
```
...
With no arguments, reads all of stdin until EOF and blocks until the stream closes — either a
...
- `bytes`: the maximum number of bytes to read before returning. Accepts a plain `number` or a
`FileSize` (from `@std/fs/filesize`). Reading stops once this many bytes have been read, even
if the stream is still open.
- `timeout`: a `Duration` (from `@std/time`) to wait for input before returning. Reading stops
once the timeout elapses, even if the stream is still open.
...
- A `boolean` `eof` that is `true` if the stream reached EOF, and `false` if reading stopped
before EOF (i.e. because the `bytes` limit was reached or the `timeout` elapsed and there may
be more to read).
...
local chunk, eof = input.read(filesize.kilobytes(1), time.seconds(5))
if not eof then
