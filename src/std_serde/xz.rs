...
fn compress(contents: &[u8], level: Option<u32>, function_name: &'static str) -> LuaResult<Vec<u8>> {
    ...
    if let Some(level) = level {
        builder = builder.compression_level(archive::CompressionLevel::Xz(level));
    }

    let compressed = match builder.build_single_file("data", contents, archive::ArchiveFormat::Xz) {
        Ok(contents) => contents,
        Err(archive::ArchiveError::InvalidCompressionLevel(reason)) => {
            return wrap_err!("{}: invalid level for xz compression; {}", function_name, reason);
        },
        Err(err) => {
...
fn xz_compress(luau: &Lua, mut multivalue: LuaMultiValue) -> LuaValueResult {
    let function_name = "xz.compress(content: string | buffer, level: number?)";
